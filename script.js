...
const hourlyTimeLabels = [
  ...
  const summaryHealthEl = document.getElementById('summary-health');
  const summaryProductivityEl = document.getElementById('summary-productivity');
  const summaryAbsenteeismEl = document.getElementById('summary-absenteeism');
  ...
  async function loadAndProcessCSV() {
      try {
          ...
          uniqueZoneIds.forEach(zoneIdStr => {
              ...
              // Per-day sums of each SBS risk factor, used to build the pie chart by day
              const dailyRiskMap = {};
              let hold = 0;
              ...
              zoneRows.forEach(row => {
                ...
                // Accumulate per-day risk factor sums for the pie chart
                if (!dailyRiskMap[dayString]) {
                  dailyRiskMap[dayString] = { co2: 0, ta: 0, spl: 0, lux: 0, count: 0 };
                }
                const dr = dailyRiskMap[dayString];
                dr.co2 += parseFloat(row.co2_risk) || 0;
                dr.ta  += parseFloat(row.ta_risk)  || 0;
                dr.spl += parseFloat(row.spl_risk) || 0;
                dr.lux += parseFloat(row.lux_risk) || 0;
                dr.count += 1;
              });
              ...
              Object.keys(dailyCountsMap).forEach(dayString => {
                ...
                // Per-day SBS causes (pie chart): normalise the 4 risk-factor sums to 100%.
                // Falls back to the zone-wide sbsCauses if the day has zero risk contributions.
                const dr = dailyRiskMap[dayString];
                let daySbsCauses;
                if (dr && (dr.co2 + dr.ta + dr.spl + dr.lux) > 0) {
                  const total = dr.co2 + dr.ta + dr.spl + dr.lux;
                  daySbsCauses = {
                    'CO₂':   parseFloat(((dr.co2 / total) * 100).toFixed(1)),
                    'Temp':  parseFloat(((dr.ta  / total) * 100).toFixed(1)),
                    'Noise': parseFloat(((dr.spl / total) * 100).toFixed(1)),
                    'Light': parseFloat(((dr.lux / total) * 100).toFixed(1)),
                  };
                  // Close rounding gap by nudging the largest slice
                  const currentSum = Object.values(daySbsCauses).reduce((s, v) => s + v, 0);
                  if (Math.abs(currentSum - 100) > 0.01) {
                    let largestCause = null; let largestVal = -1;
                    for (const c in daySbsCauses) { if (daySbsCauses[c] > largestVal) { largestVal = daySbsCauses[c]; largestCause = c; } }
                    if (largestCause) daySbsCauses[largestCause] = parseFloat((daySbsCauses[largestCause] + (100 - currentSum)).toFixed(1));
                  }
                } else {
                  daySbsCauses = { ...sbsCauses };
                }
                rep.sbsCauses = daySbsCauses;
  ...
  function createSbsPieChart(sbsCausesData) {
    console.log('hhhhh', sbsCausesData, dashboardPanelMain.classList)
      ...
      const layout = {
          ...plotlyLayoutDefaults,
          showlegend: true,
          legend: { orientation: 'h', x: 0.5, xanchor: 'center', y: -0.05, font: { size: 9 } },
          margin: { t: 5, b: 20, l: 5, r: 5 },
  ...
  function updateDashboardInfo() {
      ...
      const daySbs = data.dailyIEQData && data.dailyIEQData[currentDateIndex] && data.dailyIEQData[currentDateIndex].sbsCauses;
      createSbsPieChart(daySbs || data.sbsCauses);
  }
  ...
  function updateDateRangeChip() {
      ...
      const label = dailyDateLabels[currentDateIndex];
      if (label) dateRangeText.textContent = label;
  }

  function badgeFor(metric, value) {
      // Returns { label, className } for a coloured pill next to the summary stat.
      if (metric === 'health') {
          if (value >= 70) return { label: 'Good', className: 'good' };
          if (value >= 50) return { label: 'Average', className: 'average' };
          return { label: 'Poor', className: 'poor' };
      }
      if (metric === 'productivity') {
          if (value >= 10) return { label: 'Good', className: 'good' };
          if (value >= 5)  return { label: 'Average', className: 'average' };
          return { label: 'Poor', className: 'poor' };
      }
      if (metric === 'absenteeism') {
          // Negative = reduction (good). Positive = increase (poor).
          if (value <= -2) return { label: 'Good', className: 'good' };
          if (value <= 1)  return { label: 'Average', className: 'average' };
          return { label: 'Poor', className: 'poor' };
      }
  ...
  function updateFloorSummary() {
      if (!summaryHealthEl || !summaryProductivityEl || !summaryAbsenteeismEl) return;
      const zoneIds = Object.keys(zoneData);
      if (zoneIds.length === 0) return;

      let sumProd = 0, sumSbs = 0, sumAbs = 0, n = 0;
      zoneIds.forEach(zid => {
          const prod = calculateDailyAverage(zid, currentDateIndex, 'productivityData');
          const sbs  = calculateDailyAverage(zid, currentDateIndex, 'sbsRiskTimeSeriesData');
          const abs  = (zoneData[zid] && zoneData[zid].absenteeism) || 0;
          sumProd += prod;
          sumSbs  += sbs;
          sumAbs  += abs;
          n += 1;
      ...
      const avgSbs  = sumSbs  / n;
      const avgAbs  = sumAbs  / n;
      const healthScore = Math.max(0, Math.min(100, Math.round(100 - avgSbs)));
      const prodScore   = Math.round(avgProd);
      const absScore    = parseFloat(avgAbs.toFixed(1));

      const hBadge = badgeFor('health', healthScore);
      const pBadge = badgeFor('productivity', prodScore);
      const aBadge = badgeFor('absenteeism', absScore);

      summaryHealthEl.innerHTML        = `${healthScore} <span class="stat-badge ${hBadge.className}">${hBadge.label}</span>`;
      summaryProductivityEl.innerHTML  = `${prodScore} <span class="stat-badge ${pBadge.className}">${pBadge.label}</span>`;
      const absPrefix = absScore > 0 ? '+' : '';
      summaryAbsenteeismEl.innerHTML   = `${absPrefix}${absScore}% <span class="stat-badge ${aBadge.className}">${aBadge.label}</span>`;
  }
...
if (
  ...
  // --- Floor Pager (visual only; only Floor 0 has data) ---
  const floorPager = document.querySelector('.floor-pager');
  if (floorPager) {
    floorPager.addEventListener('click', (e) => {
      const item = e.target.closest('.floor-pager-item, .floor-pager-active');
      if (!item) return;
      const clicked = item.textContent.trim();
      floorPager.querySelectorAll('.floor-pager-active, .floor-pager-item').forEach(el => {
        el.classList.remove('floor-pager-active');
        el.classList.add('floor-pager-item');
      });
  ...
  (function setupHeaderScrollBehaviour() {
